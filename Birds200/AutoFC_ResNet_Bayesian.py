...
def get_model(num_layers, num_neurons, dropout, activation, weight_initializer, num_conv_layers,
                conv_filter_size, conv_num_filters, stride_size, pool_size):
    ...
    X = base_model.layers[-151].output # removing the last fully connected layers

    # add the conolutional layers and max pooling layers
    for i in range(num_conv_layers):
        X = layers.Conv2D(filters=conv_num_filters, kernel_size=(conv_filter_size, conv_filter_size))(X)
...
for combo in p_space:
    ...
    bounds.append({'name': 'num_conv_layers', 'type': 'discrete', 'domain': [i for i in range(1, 4)]})
    bounds.append({'name': 'conv_filter_size', 'type': 'discrete', 'domain': [i for i in range(15, 21)]})
    bounds.append({'name': 'conv_num_filters', 'type': 'discrete', 'domain': [2 ** j for j in range(5, 7)]})
    bounds.append({'name': 'stride_size', 'type': 'discrete', 'domain': [2, 3]})
    bounds.append({'name': 'pool_size', 'type': 'discrete', 'domain': [2, 3]})
    ...
    def model_fit(x):
        ...
        neurons = [int(x[:, i]) for i in range(num_layers, 2 * num_layers)] # get the values of num_neurons currently selected
        num_conv_layers = int(x[:, 2 * num_layers])
        conv_filter_size = int(x[:, 2 * num_layers + 1])
        conv_num_filters = int(x[:, 2 * num_layers + 2])
        stride_size = int(x[:, 2 * num_layers + 3])
        pool_size = int(x[:, 2 * num_layers + 4])
