...
reduce_LR = callbacks.ReduceLROnPlateau(monitor='val_loss', factor=np.sqrt(0.01), cooldown=0, patience=5, min_lr=0.5e-10)

# adagrad optimizer
ADAM = optimizers.Adam(lr=0.006, beta_1=0.9, beta_2=0.999, amsgrad=False)
...
k = 0
while k < len(bounds):
    optim_neurons.append(int(opt_.x_opt[k]))
    k += 1
    optim_dropouts.append(float(opt_.x_opt[k]))
    k += 1
