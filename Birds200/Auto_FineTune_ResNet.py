...
train_generator = train_datagen.flow_from_directory(TRAIN_PATH, target_size=(img_width, img_height), batch_size=2)
valid_datagen = ImageDataGenerator()
valid_generator = valid_datagen.flow_from_directory(VALID_PATH, target_size=(img_width, img_height), batch_size=2)
...
for i in reversed(range(1,len(model.layers))):
    ...
    predictions = Dense(10, activation="softmax")(x)
    model_final = Model(inputs = model.input, outputs = predictions)
    model_final.compile(loss = "categorical_crossentropy", optimizer = keras.optimizers.Adam(lr = 1e-6), metrics=["accuracy"])
    if flag == 1:
    ...
    for epoch in range(1,41):
        print('epoch ',epoch)
        history = model_final.fit_generator(generator=train_generator,epochs=1,validation_data = valid_generator,steps_per_epoch = 200, validation_steps = 50,verbose=2)
        train_acc = history.history['acc'][0]
        train_acc_lst.append(train_acc)
        val_acc = history.history['val_acc'][0]
        val_acc_lst.append(val_acc)
        f.write(str(i)+','+ str(epoch) +','+ str(train_acc) + ',' + str(val_acc) + '\n')
        if val_acc > max_acc:
            max_acc = val_acc
            fname = 'Auto_fine_tune_sequential.h5'
            model_final.save(fname)
    layer_acc_lst.append([train_acc_lst,val_acc_lst])
    #max_layer_acc = max(val_acc_lst)
    #if max_layer_acc > prev_max:
    #    prev_max = max_layer_acc 
   # else:
    #    break
f.close()
