...
RESULTS_PATH = os.path.join("AutoFC_NASNet", "AutoFC_NASNet_log_" + DATA_FOLDER + "_random_search_v1.csv") # The path to the results file
...
# The EarlyStopping callback stops training the model if there is no significant change in the validation accuracy
early_callback = callbacks.EarlyStopping(monitor="val_acc", patience=5, mode="auto")
# The ReduceLROnPlateau callback reduces the learning rate of the model when there is no significant change in the loss of the model
...
for i in num_layers:
    ...
    for j in temp_store: # iterate over the list of sampled configurations
        ...
        base_model = NASNetMobile(weights="imagenet") # make an instance of an existing architecture to start the experiment
        ...
        history = new_model.fit_generator(train_generator,
            validation_data=valid_generator, epochs=20, callbacks=[reduceLR_callback],
