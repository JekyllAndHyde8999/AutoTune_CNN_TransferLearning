...
RESULTS_PATH = os.path.join("AutoFC_ResNet", "AutoFC_ResNet_log_" + DATA_FOLDER + "_random_search_v1.csv")
...
for i in num_layers:
    ...
    for z in range(NUMBER_OF_SAMPLES): # randomly sample combinations
        ...
        while use_now in used_seq: # ensure the sample configurations are not repeated
    ...
    for j in temp_store: # iterate over the list of sampled configurations
        ...
        history = new_model.fit_generator(train_generator,
            validation_data=valid_generator, epochs=20, callbacks=[early_callback],
