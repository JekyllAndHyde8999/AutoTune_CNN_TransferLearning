...
DATA_FOLDER = "UCM_Land"
TRAIN_PATH = os.path.join(DATA_FOLDER, "training") # Path for training data
VALID_PATH = os.path.join(DATA_FOLDER, "validation") # Path for validation data
NUMBER_OF_CLASSES = len(os.listdir(TRAIN_PATH)) # Number of classes of the dataset
RESULTS_PATH = os.path.join("AutoFC_MobileNet", "AutoFC_MobileNet_log_ucm-land_random_search_v1.csv") # The path to the results file
...
for i in num_layers:
    ...
    for j in temp_store: # iterate over the list of sampled configurations
        ...
        base_model = MobileNet(weights="imagenet") # make an instance of an existing architecture to start the experiment
        for layer in base_model.layers: # freeze the existing layers to make them non-trainable
            layer.trainable = False

        X = base_model.layers[-6].output # removing the last fully connected layers
        ...
        history = new_model.fit_generator(train_generator,
            validation_data=valid_generator, epochs=20, callbacks=[],
